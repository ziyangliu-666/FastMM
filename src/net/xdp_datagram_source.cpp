...
int XdpDatagramSource::create_socket(detail::XskSocket& s,
  ...
  const int completion = 64;  // required by bind; nothing is ever transmitted
...
int XdpDatagramSource::open(const XdpConfig& cfg) {
  ...
  for (const auto& sub : cfg.subscriptions) {
    if ((ntohl(sub.group) >> 28U) != 0xE)
      return fail(-EINVAL, "af_xdp: " + ip_text(sub.group) + " is not an IPv4 multicast group");
