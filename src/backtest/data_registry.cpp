...
[[noreturn]] void bad(const std::string& what) { throw std::runtime_error("data: " + what); }
...
DataSourceOptions DataSourceOptions::parse(std::string_view spec,
  ...
  while (at <= rest.size() && !rest.empty()) {
    ...
    const std::string_view seg =
        trim(rest.substr(at, comma == std::string_view::npos ? std::string_view::npos : comma - at));
    ...
    if (eq == std::string_view::npos) {
      ...
      if (key.empty()) bad("'" + std::string(o.name_) + "': empty option name in '" +
                           std::string(seg) + "'");
    }
