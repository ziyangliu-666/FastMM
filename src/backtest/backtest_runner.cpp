...
struct BacktestSession::Impl final : sim::SimObserver {
  Impl(const InstrumentTable& instruments, std::size_t bars) : ledger(instruments) {
  ...
  void on_fill(const OrderFillMsg& f, Timestamp ts, Price mid) override {
...
BacktestSession::BacktestSession(const BacktestConfig& cfg, MdSource* source)
  ...
  const std::size_t bars = source_ == synthetic_.get() || generator_ != nullptr
                               ...
                               : kReserveBars;
  impl_ = std::make_unique<Impl>(cfg_.instruments, bars);
...
BacktestResult BacktestSession::run(const sim::EngineHooks& hooks,
  ...
  for (;;) {
    const bool more = driver.run_until(bar_end);
    impl_->sample(more ? bar_end : b.clock.now(), b.transport, cfg_.instruments);
    if (!more) break;
...
BacktestResult run_backtest(const BacktestConfig& cfg,
  ...
  if (StrategyRegistry::instance().find(strategy) == nullptr)
