...
constexpr VenueKeySpec kNasdaqItchKeys[] = {
    ...
    {"order_transport",
     KeyType::String,
     false,
     "sim_ouch: kernel (TCP socket) | user_tcp (experimental user-space TCP over the rx_backend's "
     "device: AF_PACKET ring, XDP socket or DPDK port; needs user_tcp_ip) (default kernel)"},
    {"user_tcp_ip",
     KeyType::String,
     false,
     "user_tcp: the connection's own IPv4 address on the interface's subnet, not assigned to any "
     "kernel interface (af_xdp, dpdk: or the host's own with user_tcp_port)"},
    {"user_tcp_port",
     KeyType::Int,
     false,
     "user_tcp: fixed local TCP port; 0 = random per connection (default 0)"},
    {"user_tcp_interface", KeyType::String, false, "user_tcp: netdev (default: interface)"},
    {"user_tcp_gateway",
     KeyType::String,
     false,
     "user_tcp: next hop IPv4 address when the OUCH server is not on-link"},
    {"ouch_username",
     KeyType::String,
     false,
