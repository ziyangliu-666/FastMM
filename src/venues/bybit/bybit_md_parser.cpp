...
MdDecodeResult BybitMdParser::decode(std::string_view json,
  ...
  if (topic.starts_with(kBook)) {
    ...
    if (dot == std::string_view::npos) return malformed();
  ...
  std::int64_t ts = 0;
  if (root["ts"].get_int64().get(ts) != sj::SUCCESS) return malformed();
  std::string_view type;
  if (root["type"].get_string().get(type) != sj::SUCCESS) return malformed();
  const bool snapshot = type == "snapshot";

  switch (kind) {
    case Kind::Depth: {
      od::object data;
      if (root["data"].get_object().get(data) != sj::SUCCESS) return malformed();
      auto* m = reinterpret_cast<BookDeltaMsg*>(out.data());
      Level* levels = m->levels();
      od::value bids;
      if (data["b"].get(bids) != sj::SUCCESS) return malformed();
      ...
      od::value asks;
      if (data["a"].get(asks) != sj::SUCCESS) return malformed();
      ...
      std::uint64_t u = 0;
      std::uint64_t seq = 0;
      if (data["u"].get_uint64().get(u) != sj::SUCCESS) return malformed();
      if (data["seq"].get_uint64().get(seq) != sj::SUCCESS) seq = 0;
      std::int64_t cts = 0;
      if (root["cts"].get_int64().get(cts) != sj::SUCCESS) cts = ts;
    ...
    case Kind::TopOfBook: {
      ...
      od::value bids;
      if (data["b"].get(bids) != sj::SUCCESS) return malformed();
      const int nb = read_levels(bids, tmp, 2);
      od::value asks;
      if (nb < 0 || data["a"].get(asks) != sj::SUCCESS) return malformed();
      const int na = read_levels(asks, tmp + 2, 2);
      if (na < 0) return malformed();
      std::uint64_t u = 0;
      if (data["u"].get_uint64().get(u) != sj::SUCCESS) return malformed();
      std::int64_t cts = 0;
      if (root["cts"].get_int64().get(cts) != sj::SUCCESS) cts = ts;
      ...
      if (snapshot) top = Top{};
      if (nb > 0) {
        top.bid_px = tmp[0].qty.is_zero() ? Price{} : tmp[0].price;
        top.bid_qty = tmp[0].qty;
      }
      if (na > 0) {
        top.ask_px = tmp[2].qty.is_zero() ? Price{} : tmp[2].price;
        top.ask_qty = tmp[2].qty;
      }
    ...
    case Kind::Trades: {
      ...
      for (auto item : data) {
        ...
        if (t["i"].get_string().get(id) != sj::SUCCESS) return malformed();
        if (t["T"].get_int64().get(trade_time) != sj::SUCCESS) return malformed();
        if (t["p"].get_string().get(p) != sj::SUCCESS) return malformed();
        if (t["v"].get_string().get(v) != sj::SUCCESS) return malformed();
        if (t["S"].get_string().get(side) != sj::SUCCESS) return malformed();
