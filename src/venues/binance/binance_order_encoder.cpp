...
// A request's parameters, inserted in alphabetical order by the call sites. Text values are
// copied into a flat arena so the signature payload and the JSON see identical bytes.
struct BinanceOrderEncoder::ParamList {
  struct Param {
    std::string_view key;
    std::string_view value;
    bool numeric;  // emitted unquoted in JSON (timestamp, recvWindow, orderId)
  };
  std::array<Param, 20> items{};
  std::size_t count = 0;
  char arena[512];
  std::size_t used = 0;
  bool ok = true;
...
bool BinanceOrderEncoder::finish_rest(ParamList& params, RestRequest& out) {
  ...
  return signer_.sign_query(out.query);
}
