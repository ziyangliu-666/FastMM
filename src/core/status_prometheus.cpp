...
class Exposition {
 public:
  ...
  void family(std::string_view name, std::string_view type, std::string_view help) {
    fmt::format_to(std::back_inserter(out_), "# HELP {} {}\n# TYPE {} {}\n", name, help, name,
                   type);
  }
...
std::string format_status_prometheus(const StatusSnapshot& s, std::int64_t now_ns) {
  ...
  e.gauge("fastmm_realized_pnl", "realized PnL, quote currency",
          static_cast<double>(s.realized_pnl_raw) * kRawToQuote);
  e.gauge("fastmm_unrealized_pnl", "unrealized PnL, quote currency",
          static_cast<double>(s.unrealized_pnl_raw) * kRawToQuote);
  e.gauge("fastmm_fees", "fees paid, quote currency",
          static_cast<double>(s.fees_raw) * kRawToQuote);
  ...
  e.family("fastmm_rejects_by_reason_total", "counter",
  ...
  e.family("fastmm_latency_quantile_seconds", "gauge",
  ...
  for (std::size_t i = 0; i < static_cast<std::size_t>(LatencyInterval::Count); ++i)
    latency_quantiles(e, "fastmm_latency_quantile_seconds",
  ...
  auto per_venue = [&](std::string_view name, std::string_view type, std::string_view help,
                       auto&& value) {
    e.family(name, type, help);
    for (std::size_t i = 0; i < venues; ++i)
      e.value_of(name,
                 fmt::format("venue=\"{}\"", label(name_of(s.venues[i].name,
                                                           sizeof s.venues[i].name))),
                 value(s.venues[i]));
  };

  e.family("fastmm_venue_channel_state", "gauge",
  ...
  for (std::size_t i = 0; i < venues; ++i) {
    ...
    for (const auto& [channel, state] : {std::pair<const char*, std::uint8_t>{"md", v.md},
                                         {"user", v.user},
                                         {"order", v.order}})
  ...
  per_venue("fastmm_venue_killed", "gauge", "1 while this venue's kill switch is engaged",
            [](const StatusVenue& v) { return static_cast<double>(v.killed ? 1 : 0); });
  per_venue("fastmm_venue_books_synced", "gauge", "books in sync",
            [](const StatusVenue& v) { return static_cast<double>(v.books_synced); });
  per_venue("fastmm_venue_books_total", "gauge", "books subscribed",
            [](const StatusVenue& v) { return static_cast<double>(v.books_total); });
  per_venue("fastmm_venue_clock_offset_seconds", "gauge", "venue clock minus the local clock",
            [](const StatusVenue& v) { return static_cast<double>(v.clock_offset_ms) * 1e-3; });
  per_venue("fastmm_venue_md_messages_total", "counter", "market-data messages",
            [](const StatusVenue& v) { return v.md_messages; });
  per_venue("fastmm_venue_resyncs_total", "counter", "book resynchronisations",
            [](const StatusVenue& v) { return v.resyncs; });
  per_venue("fastmm_venue_orders_sent_total", "counter", "new orders sent to this venue",
            [](const StatusVenue& v) { return v.orders_sent; });
  per_venue("fastmm_venue_cancels_sent_total", "counter", "cancels sent to this venue",
            [](const StatusVenue& v) { return v.cancels_sent; });
  per_venue("fastmm_venue_replaces_sent_total", "counter", "replaces sent to this venue",
            [](const StatusVenue& v) { return v.replaces_sent; });
  per_venue("fastmm_venue_order_events_total", "counter", "order events received",
            [](const StatusVenue& v) { return v.order_events; });
  per_venue("fastmm_venue_reconnects_total", "counter", "connection re-establishments",
            [](const StatusVenue& v) { return v.reconnects; });
  per_venue("fastmm_venue_rest_errors_total", "counter", "REST errors",
            [](const StatusVenue& v) { return v.rest_errors; });
  per_venue("fastmm_venue_rate_limit_cooldowns_total", "counter", "rate-limit cooldowns",
            [](const StatusVenue& v) { return v.rate_limit_cooldowns; });

  e.family("fastmm_venue_tick_to_trade_quantile_seconds", "gauge",
           "socket read to order write, measured on the network thread");
  for (std::size_t i = 0; i < venues; ++i)
    latency_quantiles(e, "fastmm_venue_tick_to_trade_quantile_seconds",
                      fmt::format("venue=\"{}\"",
                                  label(name_of(s.venues[i].name, sizeof s.venues[i].name))),
                      s.venues[i].wire_tick_to_trade);

  const bool multicast = std::any_of(s.venues, s.venues + venues,
                                     [](const StatusVenue& v) { return v.feed.state != 0; });
  if (!multicast) return e.take();

  auto per_feed = [&](std::string_view name, std::string_view type, std::string_view help,
                      auto&& value) {
    e.family(name, type, help);
    for (std::size_t i = 0; i < venues; ++i) {
  ...
  per_feed("fastmm_feed_state", "gauge",
           "multicast feed state: 1 down, 2 snapshot, 3 live, 4 lost",
           [](const StatusFeed& f) { return static_cast<double>(f.state); });
  per_feed("fastmm_feed_packets_total", "counter", "MoldUDP64 packets accepted",
           [](const StatusFeed& f) { return f.packets; });
  per_feed("fastmm_feed_gaps_total", "counter", "sequence gaps declared",
           [](const StatusFeed& f) { return f.gaps; });
  per_feed("fastmm_feed_recovered_total", "counter", "messages delivered from re-requests",
           [](const StatusFeed& f) { return f.recovered; });
  per_feed("fastmm_feed_unrecovered_total", "counter", "sequences given up on",
           [](const StatusFeed& f) { return f.unrecovered; });
  per_feed("fastmm_feed_malformed_total", "counter", "datagrams that were not MoldUDP64 packets",
           [](const StatusFeed& f) { return f.malformed; });
  per_feed("fastmm_feed_book_errors_total", "counter", "L3 book inconsistencies",
           [](const StatusFeed& f) { return f.book_errors; });
  e.family("fastmm_feed_line_duplicates_total", "counter",
