...
|---|---:|
| L2 book update near top of book | {{L2_UPDATE}} |
| Pre-trade risk check (15 checks) | {{RISK}} |
| OMS submit → ack → fill lifecycle | {{OMS}} |
| SPSC hop between two cores | {{SPSC}} |
| Tick-to-order in simulation (book delta in → order serialized) | {{T2O}} |
| Binance depth diff JSON → normalized event (20 levels) | {{JSON}} |
