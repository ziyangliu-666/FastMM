...
void* counted_alloc(std::size_t n, std::size_t align) {
  ...
  ++t_stats.allocs;
  t_stats.bytes += n;
  void* p = align > alignof(std::max_align_t) ? std::aligned_alloc(align, (n + align - 1) / align * align)
                                              : std::malloc(n == 0 ? 1 : n);
...
void* operator new(std::size_t n, const std::nothrow_t&) noexcept {
  try { return counted_alloc(n, alignof(std::max_align_t)); } catch (...) { return nullptr; }
}
void* operator new[](std::size_t n, const std::nothrow_t&) noexcept {
  try { return counted_alloc(n, alignof(std::max_align_t)); } catch (...) { return nullptr; }
}
...
AllocStats alloc_stats() noexcept { return t_stats; }
void set_alloc_trap(bool enabled) noexcept { t_trap = enabled; }
...
std::uint64_t NoAllocScope::allocations_so_far() const noexcept { return t_stats.allocs - start_.allocs; }
