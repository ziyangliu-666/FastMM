...
SessionFiles write_config(const ServerFixture& fx,
  ...
  replace_all(R"(api_key = "${FASTMM_SIM_API_KEY}")", std::string(R"(api_key = ")") + kApiKey + "\"");
...
pid_t spawn_live(const SessionFiles& f, int duration_s) {
  ...
  const int rc = posix_spawn(
      &pid, FASTMM_LIVE_EXE, nullptr, nullptr, const_cast<char* const*>(argv), environ);
  ...
  const bool placed = wait_until(
      [&, attempt = std::uint64_t{0}]() mutable {
        ...
        const venues::HttpReply r =
            http.request("POST",
                         "/api/v3/order?" + query + "&signature=" +
                             std::string(net::hmac_sha256_hex(kApiSecret, query).view()),
...
TEST_CASE("recovery: a SIGKILLed session leaves orders behind and the next one clears them") {
  ...
  if (std::find(orphans.begin(), orphans.end(), orphan) == orphans.end())
  ...
  MESSAGE("venue: accepted=" << ss.orders_accepted << " fills=" << ss.fills
                             << " orphans=" << orphans.size()
                             << " carry=" << after_second.carry().raw);
}
