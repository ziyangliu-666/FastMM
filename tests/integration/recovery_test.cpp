...
class OmsMirror {
 public:
  ...
  void request_cancel(ClientOrderId id) {
    REQUIRE(oms_.request_cancel(oms_.find(id)).has_value());
  }
  ...
  [[nodiscard]] std::vector<ClientOrderId> open_ids() const {
    ...
    std::sort(out.begin(), out.end(), [](ClientOrderId a, ClientOrderId b) {
...
TEST_CASE("recovery: the order channel is cut with an order resting, in flight and being cancelled") {
  ...
  SUBCASE("an order is in flight") {
    ...
    const OutNewOrderMsg o =
        new_order(cid(2), Side::Buy, OrderType::PostOnly, TimeInForce::Gtc,
                  resting_bid(fx, 120), kLot);
    m.submit(o);
    h.send(o.hdr);
...
TEST_CASE("recovery: an order whose response never arrives is settled by reconciliation") {
  ...
  const OutNewOrderMsg o = new_order(cid(1), Side::Buy, OrderType::PostOnly, TimeInForce::Gtc,
                                     resting_bid(fx, 100), kLot);
  m.submit(o);
  h.send(o.hdr);
  REQUIRE(h.pump([&] { return fx.server.stats().orders_accepted >= 1; }));
...
TEST_CASE("recovery: a response that arrives after the engine gave up does not resurrect an order") {
  ServerFixture fx(quiet_server());
  ReadyHarness h(fx);
  OmsMirror m(h.instruments);
...
TEST_CASE("recovery: a rate limit costs an order, not the engine's view of it") {
  ...
  const OutNewOrderMsg limited = new_order(cid(1), Side::Buy, OrderType::PostOnly,
                                           TimeInForce::Gtc, resting_bid(fx, 100),
                                           kLot);
  m.submit(limited);
  h.send(limited.hdr);
  REQUIRE(h.pump([&] {
...
TEST_CASE("recovery: clock skew past recvWindow is learned and the orders flow again") {
  ...
  const OutNewOrderMsg o2 = new_order(cid(2), Side::Buy, OrderType::PostOnly, TimeInForce::Gtc,
                                      resting_bid(fx, 110), kLot);
  m.submit(o2);
  h.send(o2.hdr);
  REQUIRE(h.pump([&] {
    m.drain(h.oc);
...
TEST_CASE("recovery: a 418 hard stop and a revoked key stop new orders but never the cancels") {
  SUBCASE("418") {
    ...
    const OutNewOrderMsg o = new_order(cid(2), Side::Buy, OrderType::PostOnly, TimeInForce::Gtc,
                                       resting_bid(fx, 110), kLot);
    m.submit(o);
    h.send(o.hdr);
    REQUIRE(h.pump([&] {
