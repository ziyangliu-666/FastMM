...
TEST_CASE("sim_exchange e2e: market-data drop pulls quotes and order-channel loss cancels all") {
  ...
  CHECK(s.min_open_orders_since_mark == 0);
  // No quote may rest at the venue during the outage (checked above). Whether that takes cancels
  // depends on timing: quotes still awaiting their ack are cancelled once acknowledged, and on a
  // busy simulator they often fill first (a CI run had 20 fills and no cancel), so the cancel
