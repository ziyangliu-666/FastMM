...
TEST_CASE("backtest.data: a spec is a name, positional arguments and key=value options") {
  ...
  const DataSourceOptions o = DataSourceOptions::parse("binance:BTCUSDT,2024-03-27,market=spot",
                                                       positional);
...
TEST_CASE("backtest.data: tardis rows of one timestamp form one L2 message") {
  const std::string dir = write_cache(
      "bt_tardis", tardis_key("binance", "incremental_book_L2", "X", "2026-09-01", ".csv"),
      kTardisBook);
  write_cache("bt_tardis", tardis_key("binance", "trades", "X", "2026-09-01", ".csv"),
              kTardisTrades);
...
TEST_CASE("backtest.data: open_data resolves specs, paths and unknown names") {
  ...
  std::unique_ptr<MdSource> s =
      open_data("binance:BTCUSDT,2024-03-27,trades=false,dir=" + dir);
  REQUIRE(s != nullptr);
...
TEST_CASE("backtest.data: convert writes a journal that replays the same events") {
  ...
  const BacktestConfig cfg =
      BacktestConfig::single_instrument("BTCUSDT", px("0.1"), qt("0.001"));
...
TEST_CASE("backtest.data: date_range and parse_utc_time") {
  ...
  CHECK_THROWS_AS(static_cast<void>(date_range("2024-03-27", "2024-03-26")),
                  std::runtime_error);
  CHECK_THROWS_AS(static_cast<void>(date_range("27-03-2024", "27-03-2024")),
                  std::runtime_error);
