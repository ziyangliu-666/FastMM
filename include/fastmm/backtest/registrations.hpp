...
// options_mm, lead_mm, xmm) in the global StrategyRegistry, for every transport. A thin wrapper
// around fastmm::register_builtin_strategies (strategies/builtin.hpp) for the library paths that
// look strategies up by name. Idempotent; call it before starting worker threads. Returns the
// number of strategies in the registry.
