...
BACKEND=kernel; TRANSPORT=kernel; DURATION=30; RUNS=1; SPEED=4; SPIN=busy
...
echo "bench-e2e: backend=$BACKEND order_transport=$TRANSPORT spin=$SPIN duration=${DURATION}s runs=$RUNS speed=$SPEED cpus sim=$SIM_CPU engine=$ENGINE_CPU net=$NET_CPU"
