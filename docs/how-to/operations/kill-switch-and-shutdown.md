...
Venue ids follow the order of the `[venues.<name>]` tables in the config, starting at 0. The engine keeps the first reason each bit was set for (`KillReason`: `Requested`, `MaxLoss`, `TransportFull`, `JournalOverflow`, `AllVenuesKilled`, `VenueFatal`, `VenueHardStop`, `OrderRingOverflow`, `StrategyError`, `FeedLost`, `OrderIdsExhausted`, `DeadMansSwitchLost`, `GatewayMaxLoss`, `GatewayOperator`); `fastmm-top` shows the flags and reasons ([Monitoring a live session](monitor-with-fastmm-top.md)).
...
| Behind `fastmm-gateway`, an operator's `kill` on the gateway's control socket ([Run behind a gateway](run-behind-a-gateway.md#control)) | Every venue, so global | `venue <id> kill switch engaged (GatewayOperator, ...)`, then `kill switch engaged (AllVenuesKilled, ...)` | ERROR | `on_kill`, as for `GatewayMaxLoss` |
