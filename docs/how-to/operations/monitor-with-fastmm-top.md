...
`fastmm-live` writes its counters, PnL, kill-switch state, latency percentiles and each venue's
status to a memory-mapped file every 250 ms without touching the engine thread; the engine's values
in it refresh every second. The file is `/dev/shm/fastmm-<engine name>.status` unless
...
`--once` prints one frame). It shows:
...
- the session: engine and strategy name, session id, pid, dry-run flag, uptime and the state
  ...
  than 3 seconds is shown as `STALE`, which usually means the process died. Next to the state,
  `KILLED (<reason>)` means the global kill switch is engaged (the reason stays in the final
  `stopped` frame) and `VENUE KILLED` that at least one venue's switch is;
...
- engine latency per interval as count, p50, p99 and max. `decode` is the network thread's parse
  ...
  engine thread, so it includes the hand-off through the SPSC ring and, with
  `spin_mode = "adaptive"`, an idle engine's wake-up; `strategy`, `serialize` and `send` are engine
...
The layout is versioned (magic number and version field). A `fastmm-top` from another build refuses
the file with `<file> was written by a different FastMM build (status segment version <n>, this
fastmm-top reads version <m>); use fastmm-top from the same build as fastmm-live` (with `--once`,
exit code 3, as for a missing file). A `fastmm-top` from before version 2 (per-reason rejects)
keeps showing `waiting for <file>` for a newer session.

## Reject logging

Rejects are counted per reason; logging is rate limited. A risk reject is logged at WARN the first
time its reason occurs, then at most once per reason every 10 seconds
