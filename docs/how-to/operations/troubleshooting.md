...
|---|---|---|
| No orders at all (`orders=0`) | `--dry-run` (quoting is disabled), the book is not synced, or a risk limit refuses every order (`risk_rejects` grows) | Check the status line and `risk_rejects=` in the summary |
