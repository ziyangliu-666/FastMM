...
| `config: unknown key '<section>.<key>' ignored (line <n>)` | Neither the schema nor, for a `[venues.<name>]` key, the connector its `kind` names knows the key ([Venue connectors](../../reference/venues.md#configuration-keys)) | Check the spelling against [Configuration](../../reference/configuration.md) |
