...
1. Build a portable tarball on a build host and copy it: `scripts/package-release.sh --preset release --out dist` writes `dist/fastmm-<version>-x86_64.tar.gz`. It holds four binaries (`fastmm-live`, `fastmm-replay`, `fastmm-top`, `fastmm-sim-itch`), the systemd unit, the benchmark and host scripts, and the two ITCH simulator configs. It refuses a `-march=native` build, and needs glibc at least as new as the build host's plus libssl3. Your config, `fastmm-ctl`, `fastmm-pnl`, `fastmm-gateway` and `fastmm-backtest` are not in it; copy them separately.
