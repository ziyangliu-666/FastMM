...
- Apps pass the function to `fastmm::cli::live`, `fastmm::cli::backtest` or `fastmm::cli::replay`. The built-in strategies (`basic_mm`, `avellaneda_stoikov`, `options_mm`, `lead_mm`) are always registered first.
