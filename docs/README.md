...
FastMM is a market-making engine in C++20. Backtests, replay and live trading run the same strategy code. These pages describe version 0.2.

## Run it in five minutes

```bash
pip install "fastmm-engine[hot]"                         # no build: backtests and Python strategies
fastmm init my-mm && cd my-mm && python backtest.py
```

From a checkout, with the C++ programs:
...
The backtest prints a summary table; `run-sim.sh` runs a live session against a local exchange and writes a journal you can replay. What the numbers mean: [Quick start](getting-started/quickstart.md).
...
- Backtesting: [Backtest on real BTCUSDT data](how-to/backtesting/binance-public-data.md)
...
- Operations: [Run on a testnet or Binance Demo](how-to/operations/run-on-testnet.md), [Deploy a release](how-to/operations/deploy.md), [Running this in production](how-to/operations/running-in-production.md), [Operations runbook](how-to/operations/runbook.md), [Go-live checklist](how-to/operations/go-live-checklist.md), [Operate a running session](how-to/operations/operate-a-running-session.md), [Run strategies behind a gateway](how-to/operations/run-behind-a-gateway.md), [Kill switch and shutdown](how-to/operations/kill-switch-and-shutdown.md), [Journals, replay and PnL](how-to/operations/journals-replay-pnl.md), [Query what you traded](how-to/operations/query-trading-records.md), [Monitor a session with fastmm-top](how-to/operations/monitor-with-fastmm-top.md), [Receive a multicast feed](how-to/operations/multicast-feeds.md), [Low-latency TCP](how-to/operations/low-latency-tcp.md), [Two-host benchmark](how-to/operations/two-host-benchmark.md), [Troubleshooting](how-to/operations/troubleshooting.md)
...
- Programs and files: [Command lines](reference/cli.md), [Configuration](reference/configuration.md), [Versions and compatibility](reference/compatibility.md), [Market-data sources](reference/data-sources.md), [Errors and exit codes](reference/errors.md), [Journal format](reference/journal-format.md), [Run report](reference/run-report.md), [Storage](reference/storage.md), [Status file](reference/status-file.md)
...
- [Judging a signal before writing a strategy](explanation/signal-research.md): the feature and forward-markout extractor, and what it says about a quote resting at the touch
...
- [Design records](adr/README.md): the 16 decisions behind the code
...
- [Cutting a release](contributing/releasing.md): the version, the CHANGELOG, the tag and what it publishes
