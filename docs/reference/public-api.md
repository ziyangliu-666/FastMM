...
- Use the built-in strategies (`strategies/basic_mm.hpp`, `avellaneda_stoikov.hpp`, `options_mm.hpp`, `lead_mm.hpp`, `xmm.hpp`) by name through the registry; their parameters and behaviour may change.
...
| `fastmm::codecs` | ITCH, OUCH, SoupBinTCP, MoldUDP64; FIX 4.4 with `FASTMM_CODEC_FIX=ON`, MDP 3.0 with `FASTMM_CODEC_MDP3=ON` | `fastmm::core` |
...
| `fastmm::live` | `run_live`, `cli::live`, `cli::gateway` | `fastmm::venues`, `fastmm::strategies` |
| `fastmm::lowlatency` | FastMM's code generation flags (link privately where you instantiate engines) | |

`find_package(fastmm CONFIG REQUIRED COMPONENTS live)` fails when the install was built without networking. A release install contains GCC LTO objects: build against it with the same compiler. The install has no programs or configurations. Version policy: [Versions and compatibility](compatibility.md).
