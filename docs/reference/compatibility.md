...
|---|---|---|
| Configuration keys | keys are added, renamed and removed. An unknown key is a warning with its line number, not an error, so a renamed key leaves the old one at its default ([Configuration](configuration.md)) | no schema version |
...
| Status file | the version rises with every layout change and there is no compatibility in either direction: `fastmm-top` refuses a file written by another version and `--once` exits 3, so `fastmm-top` and `fastmm-live` come from the same build ([Status file](status-file.md)) | 5 |
...
- Replace `fastmm-top` with `fastmm-live`, from the same tarball or image.
- Read the release's CHANGELOG section for renamed configuration keys before you start a session with an old config: the engine reports unknown keys but starts anyway.
