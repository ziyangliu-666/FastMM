...
A backtest answers two questions: does the strategy make money, and where does the money come from. `fastmm-backtest` prints both. Read the decomposition and the markouts before the net PnL, because a passive quoter can show a positive spread capture and a positive net PnL while losing on every fill it gets.

## What the simulator cannot tell you

The simulated venue ([Simulated exchange](../reference/sim-exchange.md)) and the synthetic market generator are a latency and book-mechanics model, not a market model. Four properties of the run are assumptions, not measurements.
...
`spread_captured_bps` is positive by construction for any passive quoter: it quotes away from the mid, so it is filled away from the mid. Only the markout says whether the fills were worth taking. A capture of +1 bps with a markout of −3 bps at 10 s means the strategy paid 4 bps for the privilege of being filled.

The horizons come from `[backtest] markout_horizons_s` (default `"1,10,60"` seconds). The mid used is the venue mid at `t + h`, read with the simulated clock stopped there; the run loop stops at every fill time plus horizon, so no interpolation and no nearest-sample substitution is involved. A fill the run could not mark is dropped from that horizon and counted, never marked at a substitute price:

* `past end` — `t + h` is after the last event of the run. Fills near the end of the data are excluded, so a 60 s markout on a 60 s run measures nothing at all and says so.
...
Because the excluded fills differ per horizon, `capture` is recomputed over the same fills as the markout of that horizon: the two columns of one row are always comparable.
...
A maker rebate in a backtest config is worth checking against the venue's published schedule before you believe a result. Binance spot pays no maker rebate at VIP 0; a configured `maker_bps = -0.5` adds 0.5 bps of pure revenue to every passive fill, which is more than most of the strategies in this repository capture gross.
...
`mid drift` is `signed qty * (final mid − mid at fill)` summed over the fills: the adverse selection over the whole run plus the mark-to-market of whatever inventory is still open at the end. `unexplained` is the difference from the ledger's own net PnL, which is fixed-point rounding on a healthy run; anything larger means the split does not describe the run and the split is wrong, not the ledger.
...
Neither strategy has a measurable gross edge. `basic_mm` captures 0.008 bps and `first_mm` 0.002 bps of traded notional gross, against a 10 bps maker fee: both lose 4 to 5 orders of magnitude more to fees than they earn from the spread, and both are net negative at any fee a real venue charges. Their markouts are within rounding of their spread capture, which is what the symmetric random-walk mid guarantees — the simulator cannot produce adverse selection that is not a queue or latency artefact, so a near-zero adverse selection here is a property of the simulator and not evidence that the strategies are not adversely selected live.
...
The earlier shipped configuration charged `maker_bps = -0.5`, a 0.5 bps maker rebate, and the same 60 s run returned a net PnL of +0.6087, of which 0.5992 (98.4 %) was the rebate and 0.0097 the spread. That is what the decomposition is for.
