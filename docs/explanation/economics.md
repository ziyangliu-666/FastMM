...
`basic_mm`, `avellaneda_stoikov` and `options_mm` are reference implementations of published quoting rules, written to show the engine's API. None has been shown to make money at a fee schedule you can get. [Running this in production](../how-to/operations/running-in-production.md) covers the operational side.
