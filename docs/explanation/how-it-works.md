...
All trading state — books, orders, positions, risk limits, timers — lives on one thread, `fm-engine`, and nothing else reads or writes it. That removes locks from the decision path and makes a session reproducible from its recording.
...
| `fm-journal` | appends the journal ring to the `.fmj` file | the journal ring |
| log sink | formats and writes log records | a ring per thread |
| main | config, reference data, start and stop, the per-second stats line, TSC recalibration, SIGINT/SIGTERM, `cancel_all` over REST | the control ring, and the venues directly |

Every queue is single-producer, single-consumer. The engine polls the rings round-robin with a per-ring cap (`[engine] max_events_per_step`), so one busy venue cannot starve another.

With `[engine] threading = "single"` (one venue only) the network thread disappears and the engine thread runs the reactor itself, so no event crosses a core between the packet read and the order write.
...
| Fills that arrive while the private stream is down are booked on every venue | Binance Spot and USDⓈ-M replay the trade history before reconciling; Bybit and Deribit compare open orders only, and their reconciliations are counted as estimates |
...
- [Economics of the shipped strategies](economics.md): why the example backtest is profitable and a live session is not.
